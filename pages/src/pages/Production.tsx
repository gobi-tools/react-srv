...
export default function Production() {
  ...
  <script type="module">... hydration script ... </script>
        ...
        <p>
          The markup is created from the component itself. The hydration script is
          created by reading the source file, <code>Page.tsx</code> or <code>Page.jsx</code> from disk.
        </p>
        <p>
          This is good for development but it's not ideal for production, firstly because the
          source code might not be present at all and secondly because reading and compiling code
          every time is not very fast.
        </p>
        <p>
          So, for production we can pre compile the necessary javascript and have it ready to go.
        </p>
        <p>
          First, we need to create a config file, <code>react-srv.config.ts</code> (or <code>.js</code>).
        </p>
        <figure>
  ...
  isProd: process.env.NODE_ENV === 'production',
        ...
        <p>
          This tells {PRODUCT_NAME} where to find all the source code that needs pre compiling and where
          to place the output.
        </p>
        <p>
          We can now reference it via the <code>bundle</code> command, in <code>package.json</code>
        </p>
        <figure>
          <pre><code>{`...
"scripts": {
